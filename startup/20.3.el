...
(defconst my-emacs-root-directory
  (cond
   ((string-equal my-location "borne") "/usr/local/share/emacs")
   ((string-equal my-location "hci") "/usr/local/gnu/emacs")
   (t "/usr/local/emacs"))
  "Where is emacs installed on this machine?")

(defconst my-site-lisp-directory
  (concat my-emacs-root-directory "/site-lisp")
  "Where is the local site lisp directory?")
...
(setq
 load-path (append
	    (list (concat my-site-lisp-directory "/vm"))
	    load-path))
...
(setq
 Info-directory-list
 (cond
  ((string-equal my-location "borne") (list (expand-file-name "~/local/info") "/usr/local/info"))
  ((string-equal my-location "hci") (list "/usr/local/gnu/emacs/site-info" "/usr/local/gnu/emacs/info"))
  (t "/usr/local/emacs")))
...
(when running-x
  ...
  (setq
   ...
   default-frame-alist (append default-frame-alist '((width . 100) (height . 43)))
   initial-frame-alist (append initial-frame-alist '((width . 100) (height . 43))))
...
(setq
 ...
 calendar-latitude                41.3853
 calendar-longitude               2.1774
...
(and
 ...
 (add-hook 'mail-setup-hook 'flyspell-mode-on))
