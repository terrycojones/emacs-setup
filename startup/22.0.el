...
(defconst i-use-ess                      t "True if I use ESS (emacs interface to R).")
...
(defconst i-use-slime                    t "True if I use Slime lisp interaction mode.")
...
(defconst i-use-python-mode              t "True if I use python-mode (peters, warsaw, etc).")
(defconst i-use-ipython                  t "True if I use IPython.")
...
(if (and
    ...
    (error "You cannot use both fsf-python-mode and python-mode"))

(if (and
     (boundp 'i-use-python-mode)
     (boundp 'i-use-ipython)
     i-use-ipython
     (not i-use-python-mode))
    (error "If you use ipython, you must also use python-mode"))
...
(setq
 load-path (append
	    ...
	    (list
		 ...
		 (concat my-site-lisp-directory "/nxml")
...
(setq pdb-path '/usr/local/bin/pydb
    ...
    (progn
      (setq
       load-path (append
                  load-path
                  (list
                   (expand-file-name "~/jon.es/src/Fluidinfo/trunk/emacs"))))
...
(setq
 ...
 (append
  '(("\\.\\([cChHl]\\)\\1?\\(\\.m4\\)?$"             . c-mode)
    ...
    ("\\.htm4?l?\\.m4$"                              . m4-mode)
    ("\\.\\(perl\\|cgi\\|ph\\|pl\\|pm\\)\\(\\.m4\\)?$"    . perl-mode)
    ("\\.\\(stklos\\|scm\\|stk\\)\\(\\.m4\\)?$"      . scheme-mode)
    ("\\.tk?$"                                       . tcl-mode)
    ("\\.save$"                                      . lisp-mode)
    ("\\.tac$"                                       . python-mode)
    ("\\.\\(cl\\|lisp\\)$"                           . common-lisp-mode)
