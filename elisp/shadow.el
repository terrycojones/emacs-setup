...
See the documentation for `list-load-path-shadows' for further information."
  ...
  (let (shadows				; List of shadowings, to be returned.
	...
	(file-dirs
	 (make-hash-table 2000 'equal))	; File names ever seen, with dirs.
	(true-names
	 (make-hash-table 50 'equal))	; Dirs ever considered.
	(files-seen-this-dir
	 (make-hash-table 100 'equal))	; Files seen so far in this dir.
	)
    ...
    (dolist (path-elt (or path load-path))
		...
		(or noninteractive
	      ...
	      (dolist (file curr-files)
			...
			(cl-puthash file t files-seen-this-dir)
